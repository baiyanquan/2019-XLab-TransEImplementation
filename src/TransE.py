    ...
    def transE(self, cI=20):
        ...
        for cycleIndex in range(cI):
            ...
            if cycleIndex % 100 == 0:
                ...
                # TODO: 存储每次的loss
                loss_y.append(self.loss)

                if switcher=='lab':
                    self.writeRelationVector("../data/lab/relationVector.txt")
                    self.writeEntilyVector("../data/lab/entityVector.txt")
                elif switcher == 'standard':
                    self.writeRelationVector("../data/standard/relationVector.txt")
                    self.writeEntilyVector("../data/standard/entityVector.txt")
    ...
    def update(self, Tbatch):
        ...
        for tripletWithCorruptedTriplet in Tbatch:
            ...
            if eg > 0:  # [function]+ 是一个取正值的函数
                ...
                copyEntityList[tripletWithCorruptedTriplet[0][2]] = norm(tailEntityVector)
                copyRelationList[tripletWithCorruptedTriplet[0][1]] = norm(relationVector)
                copyEntityList[tripletWithCorruptedTriplet[1][0]] = norm(headEntityVectorWithCorruptedTriplet)
                copyEntityList[tripletWithCorruptedTriplet[1][2]] = norm(tailEntityVectorWithCorruptedTriplet)
...
def train():
    if switcher == 'lab':
        dirEntity = "../data/lab/entity2id.txt"
        dirRelation = "../data/lab/relation2id.txt"
        dirTrain = "../data/lab/train.txt"
    elif switcher == 'standard':
        dirEntity = "../data/standard/entity2id.txt"
        dirRelation = "../data/standard/relation2id.txt"
        dirTrain = "../data/standard/train.txt"
    ...
    if switcher == 'lab':
        ...
        transE.writeEntilyVector("../data/lab/entityVector.txt")
    elif switcher == 'standard':
        transE.writeRelationVector("../data/standard/relationVector.txt")
        transE.writeEntilyVector("../data/standard/entityVector.txt")


def plot_loss_figure():
    loss_y[0] = INF
    plt.plot(loss_x, loss_y)
    plt.title('Training Times - Loss Figure')
...
def main():
    ...
    global loss_x, loss_y
    loss_x = [_ for _ in range(len(loss_y))]
    loss = {
        "loss_x": loss_x,
        "loss_y": loss_y
    }
