...
for datatyp in ['train', 'valid', 'test']:
    f = open(datapath + 'wordnet-mlj12-%s.txt' % datatyp, 'rb')
    dat = f.readlines()
    f.close()
    for i in dat:
...
f = open('../data/WN_synset2idx.pkl', 'wb+')
g = open('../data/WN_idx2synset.pkl', 'wb+')
...
f = open(datapath + 'wordnet-mlj12-definitions.txt', 'rb')
dat = f.readlines()
f.close()
...
f = open('../data/WN_synset2def.pkl', 'wb+')
g = open('../data/WN_synset2concept.pkl', 'wb+')
...
for datatyp in ['train', 'valid', 'test']:
    ...
    inpl = sp.lil_matrix((np.max(list(synset2idx.values())) + 1, len(dat)),
            dtype='float32')
    inpr = sp.lil_matrix((np.max(list(synset2idx.values())) + 1, len(dat)),
            dtype='float32')
    inpo = sp.lil_matrix((np.max(list(synset2idx.values())) + 1, len(dat)),
            dtype='float32')
    ...
    if 'data' not in os.listdir('../'):
        os.mkdir('../data')
    f = open('../data/WN-%s-lhs.pkl' % datatyp, 'wb+')
    g = open('../data/WN-%s-rhs.pkl' % datatyp, 'wb+')
    h = open('../data/WN-%s-rel.pkl' % datatyp, 'wb+')
